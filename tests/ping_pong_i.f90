  ...
  DO WHILE(ping_state .NE. 3)
    IF (ping_state .EQ. 0) THEN
      IF (rank .EQ. 0) THEN
        ...
        CALL MPI_Isend(buf, message_size, MPI_CHAR, pong_dest, 0, MPI_COMM_WORLD, request, ierr)
        CALL MPI_Wait(request, status, ierr)

        ping_count = ping_count + 1 + rank
        ping_state = 1
      ELSE
        CALL MPI_Irecv(buf, message_size, MPI_CHAR, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, request, ierr)
        CALL MPI_Wait(request, status, ierr)
    ...
    ELSE IF (ping_state .EQ. 2) THEN
      ...
      IF (ping_count .EQ. 100 + rank * 100) THEN
        ping_state = 3
      ELSE
        ping_state = 1
      END IF
